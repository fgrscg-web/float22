    ...
    def refresh_ui(self):
        ...
        if self.is_calculated:
            ...
            if hasattr(self, 'hull_only_centerlines'):
                ...
                # ==========================================
                # ✨ 3. 물리적인 1D 선분 컷팅 (x < x_cut 영역 완전 제거)
                # ==========================================
                # 중심선(x=x_cut) 자체가 지워지는 것을 방지하기 위해 0.5mm의 여유를 두고 우측(x가 큰 쪽) 유지
                ...
                for cl in self.hull_only_centerlines:
                    ...
                    if clipped.is_empty:
                        continue  # 기준선 우측(Starboard 쪽) 선분은 여기서 완전히 소멸됩니다.

                    geoms = [clipped] if clipped.geom_type == 'LineString' else \
                        list(clipped.geoms) if clipped.geom_type == 'MultiLineString' else []

                    for g in geoms:
                        if g.length > 1.0:  # 미세 찌꺼기 무시
                            cut_lines.append(g)

                            # 가시화도 유지된 좌측(Port 측) 부재들만 그려집니다.
                            x, y = g.xy
                            ax2.plot(x, y, color='black', linewidth=1.5, alpha=0.4, zorder=5)

                # ✨ 4. 가상의 Centerline 벽 생성
                # 절단면에 벽을 세워 중앙 화물창(Hold) 등이 좌측에서 완벽한 폐루프를 이룰 수 있게 마감
                min_y_global = min([min(g.xy[1]) for g in cut_lines]) if cut_lines else -1000.0
                centerline_boundary = LineString([(x_cut, min_y_global - 1000), (x_cut, max_y_global + 1000)])
                cut_lines.append(centerline_boundary)

                # ==========================================
                # ✨ 5. 폐루프(구획) 탐색 및 좌측 필터링
                # ==========================================
                raw_loops = list(polygonize(cut_lines))

                filtered_loops = []
                for poly in raw_loops:
                    # 도심의 X좌표가 x_cut 기준 '좌측(작은 쪽)'인 루프만 필터링 (< 부등호로 변경됨)
                    if poly.area >= 1000.0 and poly.centroid.x < x_cut - 10.0:
                        filtered_loops.append(poly)

                # 6. 구획 번호 정렬 규칙 부여
                AREA_THRESHOLD = 50 * 1000 * 1000  # 50 m^2 = 50,000,000 mm^2

                # ✨ '전체 도심의 Y좌표' 계산 (각 폐루프들의 면적 가중 평균을 이용해 가장 정확한 무게중심 도출)
                if filtered_loops:
                    global_centroid_y = sum(p.centroid.y * p.area for p in filtered_loops) / sum(
                        p.area for p in filtered_loops)
                else:
                    global_centroid_y = 0.0

                def loop_sort_key(poly):
                    # 1순위: 50m^2 이상인지 (False=0, True=1)
                    is_large = poly.area >= AREA_THRESHOLD

                    # 2순위: 중심점의 Y좌표 (바닥에서 위로)
                    dist_y = round(poly.centroid.y, -2)

                    # ✨ 3순위: 전체 도심 Y 좌표와의 비교를 통한 X축 정렬 방향 설정
                    if poly.centroid.y < global_centroid_y:
                        # 전체 도심보다 아래에 있으면 X값이 '큰' 것부터 먼저 번호 부여
                        # (파이썬 기본 정렬은 오름차순이므로 -를 붙여 내림차순으로 만듦)
                        sort_x = -poly.centroid.x
                    else:
                        # 전체 도심보다 위에 있으면 X값이 '작은' 것부터 먼저 번호 부여
                        sort_x = poly.centroid.x
                ...
                # 7. 기준선(Cut Line) 시각화 및 X좌표 겹침 출력
                ax2.axvline(x=x_cut, color='gray', linestyle='--', linewidth=1, zorder=20, alpha=0.7)
                unique_xs = sorted(set(round(hx, 1) for hx in highest_xs))
                xs_str = ", ".join([f"{hx}" for hx in unique_xs])

                # ==========================================
                # ✨ 8. 발견된 루프 시각화 및 구성 선분(Segments) 매핑 저장
                # ==========================================
                self.loop_segments_map = {}  # 외부에서 활용할 수 있도록 클래스 변수에 저장

                for idx, poly in enumerate(filtered_loops):
                    loop_name = f"L{idx + 1}"

                    # --------------------------------------------------
                    # [매핑 로직] 현재 루프의 테두리(Boundary)를 구성하는 부재들 찾기
                    # --------------------------------------------------
                    boundary = poly.boundary
                    constituent_lines = []

                    # 컷팅된 선분(cut_lines) 중 이 루프의 테두리와 겹치는 녀석들을 추출
                    for geom in cut_lines:
                        # 선분과 다각형 테두리가 겹치는(교차하는) 길이를 계산
                        overlap_length = boundary.intersection(geom).length

                        # 1.0mm 이상 선분이 겹친다면 이 루프를 감싸는 벽(부재)으로 인정
                        if overlap_length > 1.0:
                            constituent_lines.append({
                                'line_geometry': geom,
                                'overlap_length': overlap_length
                            })

                    # 저장해두기 (나중에 self.loop_segments_map['L1'] 등으로 꺼내 쓸 수 있음)
                    self.loop_segments_map[loop_name] = {
                        'polygon': poly,
                        'area': poly.area,
                        'centroid': (poly.centroid.x, poly.centroid.y),
                        'boundary_segments': constituent_lines
                    }
                    # --------------------------------------------------
                    ...
                    if poly.geom_type == 'Polygon':
                        ax2.fill(*poly.exterior.xy, color=rgb, alpha=0.5, zorder=8, edgecolor='black', linewidth=1.0)

                        for interior in poly.interiors:
                            ax2.fill(*interior.xy, color='white', alpha=1.0, zorder=9)
