...
class UltimateShipAnalyzer(QMainWindow):
    ...
    def process_1d_geometry(self):
        ...
        try:
            ...
            self.build_topology_and_calculate_flow()  # UI 렌더링 전 연산 수행!
    ...
    def refresh_ui(self):
        ...
        if self.is_calculated:
            ...
            # ----------------------------------------------------
            # [도면 2] 정정 전단류(Heatmap) 및 위상 시각화 뷰
            # ----------------------------------------------------
            if hasattr(self, 'cut_lines_info'):
                ...
                if hasattr(self, 'x_cut'):
                    ax2.axvline(x=self.x_cut, color='gray', linestyle='--', linewidth=1, zorder=20, alpha=0.7)

                # 3. 루프 라벨 (L1, L2 ...)
                if hasattr(self, 'loop_data'):
                    for name, data in self.loop_data.items():
                        cx, cy = data['centroid']
                        ax2.text(cx, cy, name, color='black', fontsize=10, fontweight='bold', ha='center', va='center',
                                 bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2), zorder=15)

                # 4. 🔥 Heatmap & 화살표 렌더링 (저장된 방향 100% 신뢰)
                if hasattr(self, 'qs_edges') and hasattr(self, 'shear_edges') and len(self.shear_edges) > 0:
                    ...
                    max_q = max(
                        [abs(data['end_q']) for data in self.qs_edges.values()] + [abs(data['start_q']) for data in
                                                                                   self.qs_edges.values()] + [1e-6])
                    ...
                    for i, edge in enumerate(self.shear_edges):
                        ...
                        # ✨ 계산 엔진이 저장해둔 방향(geom)을 따라 무조건 화살표를 그립니다!
                        ...
                        if vn > 1e-6:
                            ...
                            ax2.quiver(mid.x, mid.y, u[0], u[1], color='dodgerblue', scale=20, width=0.005, headwidth=5,
                                       pivot='mid', zorder=20)
                    ...
                    result_text = f"📊 [Shear Flow Results]\nMax |qs| : {max_q:.2e} mm³\nShear N.A : {self.NA_y_shear:.2f} mm"
                ...
                # 5. 마커 (가상 슬릿 & 노드)
                if hasattr(self, 'virtual_slits') and self.virtual_slits:
                    sx = [pt[0] for pt in self.virtual_slits]
                    sy = [pt[1] for pt in self.virtual_slits]
                    ax2.scatter(sx, sy, color='red', marker='X', s=120, zorder=25, label='Virtual Slit (q=0)')

                valid_nodes = self.get_topological_nodes(self.hull_only_centerlines)
                if valid_nodes:
                    filtered_nodes = [n for n in valid_nodes if n[0] <= 1e-3]
                    ax2.scatter([n[0] for n in filtered_nodes], [n[1] for n in filtered_nodes], color='red', s=30,
                                zorder=30, label='Topological Nodes')
    ...
    def build_topology_and_calculate_flow(self):
        """[신규 아키텍처] UI 렌더링 전, 백그라운드에서 모든 위상을 찾고 흐름 데이터를 구축합니다."""
        print("\n--- 🧩 Building Topology & Flow Routing (Original Method) ---")
        ...
        # 3. 가상 슬릿 및 흐름 분배 (1d1d.md 기존 방식 완벽 복원)
        slit_map = {}
        self.virtual_slits = set()
        processed_pairs = set()
                    ...
                    for other_loop in seg['shared_with']:
                        ...
                        if pair not in processed_pairs:
                            ...
                            if shared_geoms:
                                ...
                                self.virtual_slits.add((round(mid_pt.x, 3), round(mid_pt.y, 3)))
        ...
        if 'L1' in self.loop_data and self.loop_data['L1']['area'] < AREA_THRESHOLD:
            ...
            vert_segs = [s for s in outer_segs if
                         abs(list(s['line_geometry'].coords)[-1][1] - list(s['line_geometry'].coords)[0][1]) > abs(
            ...
            if vert_segs:
                ...
                if outer_max_x:
                    ...
                    if merged.geom_type == 'MultiLineString': merged = max(list(merged.geoms), key=lambda x: x.length)
        ...
        for idx, poly in enumerate(filtered_loops):
            ...
            target_pt = slit_map.get(tuple(sorted([loop_name, next_loop]))) if next_loop else Point(
            ...
            if is_l1 and l1_start_pt:
                ...
                prev_geoms = [s['line_geometry'] for s in l_segs if prev_loop in s['shared_with']]
                merged_prev = linemerge(prev_geoms)
                if merged_prev.geom_type == 'MultiLineString': merged_prev = max(list(merged_prev.geoms),
                                                                                 key=lambda x: x.length)
                nA, nB = Point(merged_prev.coords[0]), Point(merged_prev.coords[-1])
                d_nA, d_nB = ring.project(nA), ring.project(nB)
                if (d_nA - d_ps) % L_ring < (d_nB - d_ps) % L_ring:
                    d_nr, d_nl = d_nA, d_nB
                else:
                    d_nr, d_nl = d_nB, d_nA
            else:
                lowest_node = Point(min(self.loop_data[loop_name]['nodes'], key=lambda pt: pt[1]))
            ...
            for seg in l_segs:
                ...
                if target_pt and geom.distance(target_pt) < 1.0 and geom.length > 2.0:
                    d_pt = geom.project(target_pt, normalized=True)
                    if 0.05 < d_pt < 0.95:
                        seg['is_split'] = True
                        seg['split_pt'] = (target_pt.x, target_pt.y)
                        seg['is_sink'] = True
                        handled = True
                elif not is_l1 and slit_prev and geom.distance(slit_prev) < 1.0 and geom.length > 2.0:
                    d_pt = geom.project(slit_prev, normalized=True)
                    if 0.05 < d_pt < 0.95:
                        seg['is_split'] = True
                        seg['split_pt'] = (slit_prev.x, slit_prev.y)
                        seg['is_sink'] = False
                        handled = True
                elif is_l1 and l1_start_pt and geom.distance(l1_start_pt) < 1.0 and geom.length > 2.0:
                    d_pt = geom.project(l1_start_pt, normalized=True)
                    if 0.05 < d_pt < 0.95:
                        seg['is_split'] = True
                        seg['split_pt'] = (l1_start_pt.x, l1_start_pt.y)
                        seg['is_sink'] = False
                        handled = True
        ...
        # 4. 오픈 단면(Open Segment) 배분 (자유단 -> 선체 내부 방향 고정)
        self.open_segments = []
        loop_lines = [s['line_geometry'] for data in self.loop_data.values() for s in data['segments']]
        for info in self.cut_lines_info:
            geom = info['line_geometry']
            geom_id = id(geom)
            ...
            if is_open:
                ...
                # 1. 선체 본체(폐루프)와의 거리를 측정하여 허공에 뜬 끝단(자유단) 찾기
                dist_start = min([p_start.distance(lg) for lg in loop_lines]) if loop_lines else 0
                dist_end = min([p_end.distance(lg) for lg in loop_lines]) if loop_lines else 0

                # 2. 🚨 핵심 경계조건: 무조건 자유단(먼 곳, q=0)에서 선체 조인트(가까운 곳)로 흐르도록 위상 강제!
                if dist_start > dist_end:
                    # p_start가 먼 곳(자유단)이면 p_start -> p_end 로 흐름
                    vec = np.array(coords[-1]) - np.array(coords[0])
                else:
                    # p_end가 먼 곳(자유단)이면 p_end -> p_start 로 역류
                    vec = np.array(coords[0]) - np.array(coords[-1])

                if np.linalg.norm(vec) > 1e-6:
                    u_vec = vec / np.linalg.norm(vec)
                    self.open_segments.append({
                        'line_geometry': geom,
                        'thickness': info['thickness'],
                        'flow_vec': (u_vec[0], u_vec[1])
                    })

        # 토폴로지 분석이 완벽히 끝났으므로, 즉시 전단류 계산 엔진 가동!
        self.calculate_determinate_shear_flow()

    def calculate_determinate_shear_flow(self):
        """[계산 엔진] 위상 수학적 붕괴 방지(Topology Break) 로직이 적용된 전단류 누적기"""
        ...
        def add_edge_geom(geom, thickness, is_forward=True, is_slit_seg=False, outer_node_id=None):
            if geom.length < 1e-3: return
            coords = list(geom.coords)
            if not is_forward: coords = coords[::-1]
            oriented_geom = LineString(coords)
            n_start = get_node_id(coords[0])
            n_end = get_node_id(coords[-1])
            edges.append({
                'start_node': n_start, 'end_node': n_end,
                'start': node_coords[n_start], 'end': node_coords[n_end],
                't': thickness, 'L': oriented_geom.length,
                'mid_y': oriented_geom.centroid.y, 'geom': oriented_geom,
                'is_slit_seg': is_slit_seg, 'outer_node': outer_node_id
            })
        ...
        if hasattr(self, 'loop_data'):
            for loop_name, data in self.loop_data.items():
                for seg in data['segments']:
                    ...
                    # ✨ 슬릿(가상 슬릿) 부재일 경우, 양 끝단 노드(outer_node)를 별도로 기억합니다.
                    if seg.get('is_split'):
                        orig_coords = list(geom.coords)
                        node1 = get_node_id(orig_coords[0])
                        node2 = get_node_id(orig_coords[-1])
                        ...
                        if seg.get('is_sink', True):
                            add_edge_geom(geom1, t, True, is_slit_seg=True, outer_node_id=node1)
                            add_edge_geom(geom2, t, False, is_slit_seg=True, outer_node_id=node2)
                        else:
                            add_edge_geom(geom1, t, False, is_slit_seg=True, outer_node_id=node1)
                            add_edge_geom(geom2, t, True, is_slit_seg=True, outer_node_id=node2)
                    elif 'flow_vec' in seg:
                        coords = list(geom.coords)
                        vec = np.array(coords[-1]) - np.array(coords[0])
                        f_vec = np.array(seg['flow_vec'])
                        add_edge_geom(geom, t, np.dot(vec, f_vec) > 0, is_slit_seg=False)

        if hasattr(self, 'open_segments'):
        ...
        for e in edges: e['dQ'] = e['t'] * e['L'] * (e['mid_y'] - NA_y)

        # ====================================================================
        # ✨ [위상 분리(Topology Break) 로직] - 강윤 님의 2번 룰 완벽 적용!
        # 슬릿 부재의 양 끝단 노드를 찾아, 다음 폐단면으로 나가는 흐름을 q=0으로 강제 리셋합니다.
        # ====================================================================
        slit_end_nodes = set(e['outer_node'] for e in edges if e['is_slit_seg'] and e['outer_node'] is not None)
        ...
        for i, e in enumerate(edges):
            ...
            # 만약 일반 선분(다음 루프)이 슬릿의 끝단(u)에서 출발하려고 한다면?
            if u in slit_end_nodes and not e['is_slit_seg']:
                # 이전 루프와 위상(Topology)을 끊어버리고, 새로운 가짜 노드에서 q=0으로 출발하게 만듭니다.
                new_u = len(node_coords)
                node_coords.append(node_coords[u])
                adj[new_u] = {'in': [], 'out': []}
                e['start_node'] = new_u
                u = new_u
        ...
        # 진입 흐름이 끊어진 오픈 단면 및 위상이 분리된 슬릿 끝단들이 자동적으로 q=0 출발점이 됨
        ...
        while queue:
            ...
            for edge_idx in adj[curr]['out']:
                ...
                nxt = e['end_node']
                qs_nodes[nxt] += end_q  # 연속된 브릿지 노드(Bridge Node)에서는 경로에 따라 전단류(q)가 쌓입니다.
                in_degree[nxt] -= 1
