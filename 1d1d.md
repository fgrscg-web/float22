...
class UltimateShipAnalyzer(QMainWindow):
    ...
    def refresh_ui(self):
        ...
        if self.is_calculated:
            ...
            if hasattr(self, 'final_healed_centerlines'):
                ...
                ax2.scatter([n[0] for n in valid_nodes], [n[1] for n in valid_nodes], color='red', s=30, zorder=15)
                # ------------------------------------
    ...
    def get_topological_nodes(self, centerlines, tolerance=1.0):
        ...
        def get_outward_vector(cl, p_ref):
            ...
            # 노드가 시작점인지 끝점인지 판별하여 바로 다음 좌표와의 벡터 계산
        ...
        for pt, connected_cls in node_map.items():
            # 조건 1: 3개 이상의 선분이 만나면 구조적 핵심 교차점이므로 무조건 유지
            if len(connected_cls) >= 3:
                final_nodes.append(pt)

            # 조건 2: 딱 2개의 선분만 만날 때 (탈락 검사 대상)
            elif len(connected_cls) == 2:
                cl1, cl2 = connected_cls[0], connected_cls[1]
                t1 = cl1.get('thickness', 10.0)
                t2 = cl2.get('thickness', 10.0)

                # 2-1: 두께가 다르면, 강성(Stiffness)이 변하는 지점이므로 노드 유지
                if abs(t1 - t2) > 1e-3:
                    final_nodes.append(pt)
                    continue

                # 2-2: 두께가 같다면, 두 선분이 일직선(180도)인지 각도 검사
                v1 = get_outward_vector(cl1, pt)
                v2 = get_outward_vector(cl2, pt)

                cos_theta = np.dot(v1, v2)

                # 두 벡터가 완벽히 반대 방향(180도)이면 cos_theta는 -1입니다.
                # 오차 5도(175도, cos(175) ≈ -0.996)를 허용하여, 이보다 각이 크면 코너로 인정.
                if cos_theta > -0.996:
                    # 일직선이 아님 (L자 등 코너/꺾임 부재) -> 노드 유지
                    final_nodes.append(pt)
                else:
                    # 일직선상에 있고 두께 변화도 없음 -> 🚨 노드 탈락! (Drop)
                    pass

            # 1개만 있는 경우는 단순 끝점이므로 노드로 취급하지 않음 (Pass)
