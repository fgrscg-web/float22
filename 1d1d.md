            ...
            if hasattr(self, 'hull_only_centerlines'):
                ...
                for idx, poly in enumerate(filtered_loops):
                    ...
                    for seg in l_segs:
                        ...
                        # ✨ 핵심 수정 1: 화살표를 선분에 100% 밀착시키는 고유 벡터 생성
                        coords = list(geom.coords)
                        seg_vec = np.array(coords[-1]) - np.array(coords[0])
                        if np.linalg.norm(seg_vec) < 1e-6: continue
                        u_vec = seg_vec / np.linalg.norm(seg_vec)

                        # 예외 1: L1 Start를 품은 단일 선분 (양쪽으로 빠져나감)
                        if is_l1 and l1_start_pt and geom.distance(
                                l1_start_pt) < 1.0 and geom.length > 2.0:
                            c = np.array([l1_start_pt.x, l1_start_pt.y])
                            pt1 = geom.interpolate(0.2, normalized=True)
                            pt2 = geom.interpolate(0.8, normalized=True)
                            # 중심에서 멀어지는 방향으로 부호 판별
                            flow1 = u_vec if np.dot(np.array([pt1.x - c[0], pt1.y - c[1]]),
                                                    u_vec) > 0 else -u_vec
                            flow2 = u_vec if np.dot(np.array([pt2.x - c[0], pt2.y - c[1]]),
                                                    u_vec) > 0 else -u_vec

                            ax2.quiver(pt1.x, pt1.y, flow1[0], flow1[1], color='blue', scale=15,
                                       width=0.005, headwidth=5, zorder=20)
                            ax2.quiver(pt2.x, pt2.y, flow2[0], flow2[1], color='blue', scale=15,
                                       width=0.005, headwidth=5, zorder=20)
                            continue

                        # 예외 2: Target Slit을 품은 단일 선분 (양쪽에서 슬릿으로 들어옴)
                        if target_pt and geom.distance(target_pt) < 1.0 and geom.length > 2.0:
                            c = np.array([target_pt.x, target_pt.y])
                            pt1 = geom.interpolate(0.2, normalized=True)
                            pt2 = geom.interpolate(0.8, normalized=True)
                            # 중심(슬릿)으로 들어오는 방향으로 부호 판별
                            flow1 = u_vec if np.dot(np.array([c[0] - pt1.x, c[1] - pt1.y]),
                                                    u_vec) > 0 else -u_vec
                            flow2 = u_vec if np.dot(np.array([c[0] - pt2.x, c[1] - pt2.y]),
                                                    u_vec) > 0 else -u_vec
                        ...
                        # 예외 3: 이전 공유격벽 Slit(Source)을 품은 단일 선분 (양쪽에서 멀어짐)
                        if not is_l1 and slit_prev and geom.distance(
                                slit_prev) < 1.0 and geom.length > 2.0:
                            c = np.array([slit_prev.x, slit_prev.y])
                            pt1 = geom.interpolate(0.2, normalized=True)
                            pt2 = geom.interpolate(0.8, normalized=True)
                            # 중심(이전 슬릿)에서 멀어지는 방향으로 부호 판별
                            flow1 = u_vec if np.dot(np.array([pt1.x - c[0], pt1.y - c[1]]),
                                                    u_vec) > 0 else -u_vec
                            flow2 = u_vec if np.dot(np.array([pt2.x - c[0], pt2.y - c[1]]),
                                                    u_vec) > 0 else -u_vec
                        ...
                        # 선분 고유 벡터(u_vec)가 링의 정방향을 향하도록 정렬
                        ...
                        ax2.quiver(mid_pt.x, mid_pt.y, flow_vec[0], flow_vec[1], color='blue', scale=15,
                                   width=0.005, headwidth=5, zorder=20)
                ...
                for info in cut_lines_info:
                    ...
                    if is_open:
                        ...
                        if np.linalg.norm(vec) > 1e-6:
                            ...
                            ax2.quiver(mid.x, mid.y, vec[0], vec[1], color='dodgerblue', scale=15,
                                       width=0.005, headwidth=5, zorder=20)
