...
class UltimateShipAnalyzer(QMainWindow):
    ...
    def process_1d_geometry(self):
        ...
        try:
            ...
            all_cl = split_all_lines_at_intersections(all_cl)
            all_cl = clean_topology(all_cl, trim_tol=500.0)
            all_cl = weld_vertices(all_cl, weld_tol=1.0)
    ...
    def refresh_ui(self):
        ...
        if self.is_calculated:
            ...
            if hasattr(self, 'final_healed_centerlines'):
                ...
                # 노드(교차점) 렌더링
                valid_nodes = self.get_topological_nodes(self.final_healed_centerlines)
                if valid_nodes:
                    ax1.scatter([n[0] for n in valid_nodes], [n[1] for n in valid_nodes], color='red', s=30, zorder=15)
            ...
            if hasattr(self, 'hull_only_centerlines'):
                ...
                # ==========================================
                # ✨ 9. 결정론적 흐름 분배 및 가상 슬릿 네트워크 (수정된 로직)
                # ==========================================
                from shapely.geometry import Point
                import numpy as np

                # 1. 노드와 노드 사이 경로를 line으로 간주 (모든 유효 선분 및 노드 수집)
                all_line_geoms = []
                unique_nodes = set()
                ...
                for info in cut_lines_info:
                    ...
                    coords = list(geom.coords)
                    # ✨ 오류 수정: 기존 루프 데이터 생성부와 동일하게 소수점 2자리로 통일
                    unique_nodes.add((round(coords[0][0], 2), round(coords[0][1], 2)))
                    unique_nodes.add((round(coords[-1][0], 2), round(coords[-1][1], 2)))

                cx = x_cut  # 중심선 X 좌표

                # 폐루프(Closed Loop)에 속한 노드 별도 수집
                loop_nodes_set = set()
                for loop_name, data in self.loop_data.items():
                    for seg in data['segments']:
                        coords = list(seg['line_geometry'].coords)
                        # ✨ 오류 수정: 소수점 2자리 통일
                        loop_nodes_set.add((round(coords[0][0], 2), round(coords[0][1], 2)))
                        loop_nodes_set.add((round(coords[-1][0], 2), round(coords[-1][1], 2)))

                # 4. 스타트 포인트 탐색: 중심선과 가장 가깝고, y축 좌표가 가장 작은 노드
                if unique_nodes:
                    start_node = min(unique_nodes, key=lambda pt: (pt[1], abs(pt[0] - cx)))
                else:
                    start_node = None

                # 노드별 In/Out 흐름 추적 및 Degree(연결된 부재 수) 계산
                node_flows = {pt: {'in': [], 'out': [], 'degree': 0} for pt in unique_nodes}
                flow_vectors = []

                # 2 & 3. 흐름 분배 로직 적용 (Y축 증가 및 X축 멀어지는 방향)
                for geom in all_line_geoms:
                    coords = list(geom.coords)
                    ...
                    node_flows[n1]['degree'] += 1
                    node_flows[n2]['degree'] += 1
                    ...
                    if abs(dy) > 1e-3:
                        if dy > 0:
                            start_pt, end_pt = n1, n2
                            vec = p2 - p1
                        else:
                            start_pt, end_pt = n2, n1
                            vec = p1 - p2
                    else:
                        dist1 = abs(p1[0] - cx)
                        dist2 = abs(p2[0] - cx)
                    ...
                    node_flows[start_pt]['out'].append(geom)
                    node_flows[end_pt]['in'].append(geom)

                    flow_vectors.append({
                        'geom': geom, 'vec': vec, 'n1': n1, 'n2': n2,
                        'mid': geom.interpolate(0.5, normalized=True)
                    })

                # 5. 브릿지 노드 탐색 (흐름이 모이는 교차점)
                bridge_nodes = []
                self.bridge_id_map = {}

                for pt, data in node_flows.items():
                    if data['degree'] >= 3 and len(data['in']) >= 2:
                        bridge_nodes.append(pt)
                        self.bridge_id_map[pt] = {
                            'id_in': f"Bridge_{pt[0]}_{pt[1]}_IN",
                            'id_out': f"Bridge_{pt[0]}_{pt[1]}_OUT"
                        }

                # ✨ 각 노드가 어떤 루프들에 속해 있는지 추적 (인접 루프 공유 여부 판별)
                node_to_loops = {pt: [] for pt in unique_nodes}
                for loop_name, ldata in self.loop_data.items():
                    for pt in ldata['nodes']:
                        node_to_loops[pt].append(loop_name)

                # ✨ 루프에 속한 전체 선분(Geometry) 수집 (오픈 선분 판별용)
                loop_geoms = []
                for ldata in self.loop_data.values():
                    for s in ldata['segments']:
                        loop_geoms.append(s['line_geometry'])

                def is_open_segment(geom):
                    # 폐루프를 구성하는 선분 리스트에 포함되지 않으면 오픈 선분(보강재/꼬리)으로 판별
                    for lg in loop_geoms:
                        if geom.equals_exact(lg, 1e-3):
                            return False
                    return True

                # 바깥을 향하는 방향 벡터 계산 도우미 함수
                def get_outward_vec(geom, p_ref):
                    c = list(geom.coords)
                    p1, p2 = np.array(c[0]), np.array(c[-1])
                    if np.linalg.norm(p1 - np.array(p_ref)) < 1e-3:
                        v = p2 - p1
                    else:
                        v = p1 - p2
                    v_norm = np.linalg.norm(v)
                    return v / v_norm if v_norm > 1e-6 else np.array([0, 0])

                # ✨ 5-1. 슬릿 노드 탐색: 3가지 우선순위 조건에 따른 정밀 할당
                slit_nodes = set()
                self.loop_slit_map = {}
                ...
                for loop_name, ldata in self.loop_data.items():
                    ...
                    cond1_shared_collinear = []
                    cond2_open_collinear = []
                    cond3_unshared_corner = []

                    for pt in l_nodes:
                        if pt in bridge_nodes:
                            continue  # 브릿지가 할당된 노드는 슬릿 후보에서 원천 배제

                        connected = node_flows[pt]['in'] + node_flows[pt]['out']

                        collinear_shared = False
                        collinear_open = False
                        collinear_any = False

                        # 연결된 모든 선분 쌍에 대해 일직선 여부 검사
                        for i in range(len(connected)):
                            for j in range(i + 1, len(connected)):
                                v1 = get_outward_vec(connected[i], pt)
                                v2 = get_outward_vec(connected[j], pt)

                                # 내적이 -0.95 미만이면 거의 일직선(Collinear) 형태
                                if np.dot(v1, v2) < -0.95:
                                    collinear_any = True

                                    is_open_i = is_open_segment(connected[i])
                                    is_open_j = is_open_segment(connected[j])

                                    if is_open_i or is_open_j:
                                        collinear_open = True
                                    elif len(node_to_loops[pt]) > 1:
                                        collinear_shared = True

                        # 우선순위 1. 다른 인접 루프로부터 일직선으로 이어지는 부분
                        if collinear_shared:
                            cond1_shared_collinear.append(pt)
                        # 우선순위 2. 인접한 오픈된 선분으로부터 같은 방향을 가지는 곳
                        elif collinear_open:
                            cond2_open_collinear.append(pt)
                        # 우선순위 3. 같은 폐루프 내에서 일직선상의 노드가 아닌 곳 (순수 꼭짓점)
                        elif not collinear_any and len(node_to_loops[pt]) == 1:
                            cond3_unshared_corner.append(pt)

                    # ✨ 조건별 슬릿 최종 할당 (가장 아래쪽 노드 우선)
                    if cond1_shared_collinear:
                        slit_pt = min(cond1_shared_collinear, key=lambda n: n[1])
                    elif cond2_open_collinear:
                        slit_pt = min(cond2_open_collinear, key=lambda n: n[1])
                    elif cond3_unshared_corner:
                        slit_pt = min(cond3_unshared_corner, key=lambda n: n[1])
                    else:
                        # 예외 페일세이프: 위 조건이 모두 없을 경우 브릿지가 아닌 가장 하단 노드 강제 지정
                        non_bridges = [pt for pt in l_nodes if pt not in bridge_nodes]
                        slit_pt = min(non_bridges, key=lambda n: n[1]) if non_bridges else min(l_nodes,
                                                                                               key=lambda n: n[1])

                    slit_nodes.add(slit_pt)
                    self.loop_slit_map[loop_name] = [slit_pt]

                # 6. 라인당 화살표 하나로 흐름 가시화
                for f in flow_vectors:
                    mid = f['mid']
                    vec = f['vec']
                    ...
                    if v_norm > 1e-6:
                        u_vec = vec / v_norm
                        # 선분은 더 이상 초록색으로 칠하지 않고 전체 파란색으로 통일
                        ax2.quiver(mid.x, mid.y, u_vec[0], u_vec[1], color='dodgerblue', scale=15, width=0.005,
                                   headwidth=5, zorder=20)

                # 7. 각각의 노드 역할에 따른 색상/도형 가시화 구분 및 라벨링
                if start_node:
                    ax2.scatter(start_node[0], start_node[1], color='green', marker='o', s=150, zorder=26,
                                label='Global Start (Node 1)')

                if bridge_nodes:
                    bx = [pt[0] for pt in bridge_nodes]
                    by = [pt[1] for pt in bridge_nodes]
                    ax2.scatter(bx, by, color='skyblue', marker='s', s=100, zorder=24,
                                label='Bridge Node (Combine)')

                if slit_nodes:
                    sx = [pt[0] for pt in slit_nodes]
                    sy = [pt[1] for pt in slit_nodes]
                    ax2.scatter(sx, sy, color='red', marker='^', s=130, zorder=25, label='Loop Slit (q=0)')

                    for loop_name, slits in self.loop_slit_map.items():
                        for spt in slits:
                            ax2.text(spt[0] + 60, spt[1] + 60, f"Slit({loop_name})", color='red', fontsize=10,
                                     fontweight='bold', zorder=30)

                # ✨ 요구사항 반영: Degree=1인 노드를 초록색으로 표현 (선분 말고)
                degree_1_nodes = [pt for pt, data in node_flows.items() if data['degree'] == 1]
                if degree_1_nodes:
                    dx1 = [pt[0] for pt in degree_1_nodes]
                    dy1 = [pt[1] for pt in degree_1_nodes]
                    # 시작점과 겹치지 않게 연두색(limegreen)으로 약간 작게 표시
                    ax2.scatter(dx1, dy1, color='limegreen', marker='o', s=60, zorder=23,
                                label='Open Edge (Degree 1)')
